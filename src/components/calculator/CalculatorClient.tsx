...
export default function CalculatorClient() {
    ...
    async function handleSave() {
        ...
        const record: HistoryRecord = {
            id: uuidv4(),
            date: saveDate,
            type: catalog === 'services' ? 'Serviços' : 'Enxoval',
...
<div class="header"><h1>🧺 Lavanderia</h1><p>Comanda de ${catalog === 'services' ? 'Serviços' : 'Enxoval'}</p></div>
<div class="meta">
<span>📅 ${dateStr} — ${timeStr}</span>
<span>🏷️ ${catalog === 'services' ? `Tipo: ${serviceType}` : 'Enxoval'}</span>
</div>
            ...
            <Modal open={saveModalOpen} onClose={() => setSaveModalOpen(false)} title="Salvar no Histórico">
                <div className="space-y-4">
                    ...
                    <div className="glass-card-static p-4 rounded-xl">
                        <div className="summary-row">
                            <span>Tipo</span>
                            <span className="badge badge-amber">{catalog === 'services' ? 'Serviços' : 'Enxoval'}</span>
                        </div>
                        {catalog === 'services' && (
                            <div className="summary-row">
                                <span>Serviço</span>
